...
from .pages import XhsLoginPage
...
def resolve_account_file(account_name: str, runtime_dir: Path | None = None) -> Path:
    ...
    path = resolve_runtime_dir(runtime_dir) / "cookies" / f"xhs_{safe_name}.json"
    ...
    *,
    ...
    async with build_browser_session(get_social_browser(browser), options) as session:
        ...
        try:
            ...
            try:
                if not str(page.url or "").startswith(S.UPLOAD_URL):
...
async def login_xhs(
    ...
    *,
    ...
    async with build_browser_session(get_social_browser(browser), options) as session:
        ...
        try:
            page = await context.new_page()
            login_page = XhsLoginPage(page)
            await emit(on_event, UploadStatusEvent(UploadStage.NAVIGATING, "打开小红书创作者中心"))
            await login_page.open()
            await emit(on_event, UploadStatusEvent(UploadStage.LOGIN_QRCODE, "等待小红书登录二维码出现", {"current_url": page.url}))
